...
curry (uncurry f)
                                                         = (η)
curry (λ ab → uncurry f ab)
                                                         = (uncurry def)
curry (λ ab → f (proj₁ ab) (proj₂ ab))
                                                         = (η)
λ a b → curry (λ ab → f (proj₁ ab) (proj₂ ab)) a b
                                                         = (curry def)
λ a b → (λ ab → f (proj₁ ab) (proj₂ ab)) (a , b)
                                                         = (β)
λ a b → f (proj₁ (a , b)) (proj₂ (a , b))
                                                         = (proj₁, proj₂ def)
λ a b → f a b
                                                         = (η)
f
...
-- FEL: add meg Three × Three osszes lehetseges elemet! ha tobb van, mint 4, meg vegyel hozza ujabbakat!
r1 r2 r3 r4 r5 r6 r7 r8 r9 : Three × Three
r1 = zero , zero
r2 = zero , one
r3 = zero , two
r4 = one , zero
...
case'  : (A → C) × (B → C) → A ⊎ B → C
case' h = case (proj₁ h) (proj₂ h)

-- FEL: mutasd meg egyenlosegi ervelessel, hogy uncase (case' f) = f
{- ->
uncase (case' f)
                                                                                           = (uncase def)
(λ a → case' f (inj₁ a)) , (λ b → case' f (inj₂ b))
                                                                                           = (case' def)
(λ a → case (proj₁ f) (proj₂ f) (inj₁ a)) , (λ b → case (proj₁ f) (proj₂ f) (inj₂ b))
                                                                                           = (case def)
(λ a → proj₁ f a) , (λ b → proj₂ f b)
                                                                                           = (η)
proj₁ f , proj₂ f
                                                                                           = (surjective pairing)
f
...
case' (uncase g)
                                                                                           = (case' def)
case (proj₁ (uncase g)) (proj₂ (uncase g))
                                                                                           = (uncase def, proj₁ def)
case (λ a → g (inj₁ a)) (λ b → g (inj₂ b))
                                                                                           = (η)
λ ab → case (λ a → g (inj₁ a)) (λ b → g (inj₂ b)) ab
                                                                                           ??? (ezt nem tudjuk belatni)
λ ab → g ab
                                                                                           = (η)
g
-}
...
four0 = inj₁ tt
four1 = inj₂ (inj₁ tt)
four2 = inj₂ (inj₂ (inj₁ tt))
four3 = inj₂ (inj₂ (inj₂ tt))
...
λ ab → A× id ab 
                                          =(A× def)
λ ab → proj₁ ab , id (proj₂ ab)
                                          =(id def)
λ ab → proj₁ ab , proj₂ ab
                                          =(surjective pairing)
λ ab → ab
                                          =(id def)
id                      
...
λ ab → A× (f ∘ g) ab
                                                                                    =(A× def)
λ ab → proj₁ ab , (f ∘ g) (proj₂ ab)
                                                                                    =(_∘_ def)
λ ab → proj₁ ab , f (g (proj₂ ab))
                                                                                    =(proj₂ def)
λ ab → proj₁ ab , f (proj₂ (proj₁ ab , g (proj₂ ab)))
                                                                                    =(proj₁ def)
λ ab → proj₁ (proj₁ ab , g (proj₂ ab)) , f (proj₂ (proj₁ ab , g (proj₂ ab)))
                                                                                    =(A× def)
λ ab → A× f (proj₁ ab , g (proj₂ ab))
                                                                                    =(A× def)
λ ab → A× f (A× g ab)
                                                                                    =(_∘_ def)
λ ab → ((A× f) ∘ (A× g)) ab
