...
dist = 
  (λ x → case (snd x) (λ b → inl (fst x , b))
                      (λ c → inr (fst x , c))) ,
  {!!}
{-
        A+B          A×B       ⊤     ⊥           A → B
GOAL    inl ?        ? , ?     tt                λ a → ?
        inr ?
w ASS   case w ? ?   fst w           exfalso w   w ?
                     snd w
-}
...
iso1 : {A B : Type} →
    (Bool → (A ⊎ B)) ↔                       -- (A+B)^2
    ((Bool → A) ⊎ Bool × A × B ⊎ (Bool → B)) -- A^2+2*A*B+B^2
iso1 =
  (λ f → case (f true)
    (λ a₀ → case (f false)
      (λ a₁ → inl λ t → if t then a₀ else a₁) -- A^2
      (λ b₁ → inr (inl (true , a₀ , b₁))))    -- A*B
    (λ b₀ → case (f false)
      (λ a₁ → inr (inl (false , a₁ , b₀)))    -- A*B
      (λ b₁ → inr (inr λ t → if t then b₀ else b₁)))) , -- B^2
  {!!}
